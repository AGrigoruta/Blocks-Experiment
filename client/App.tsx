...
import { createEmptyGrid, findDropPosition, validateMove, applyBlockToGrid, checkWin } from './utils/gameLogic';
import { GridState, BlockData, Player, Orientation, GameMode, NetworkRole, NetworkMessage } from './types';
import { INITIAL_CAMERA_POSITION, GRID_SIZE, INITIAL_TIME_SECONDS, INCREMENT_SECONDS } from './constants';
...
function App() {
  ...
  const [winningCells, setWinningCells] = useState<{x: number, y: number}[] | null>(null);
  ...
  const connectSocket = () => {
    ...
    socket.on('connect_error', (err) => {
      ...
      setErrorMessage("Could not connect to game server. Is 'node server.js' running?");
  ...
  const handleNetworkAction = (data: any) => {
    if (data.type === 'MOVE') {
      ...
      setGrid(prev => applyBlockToGrid(prev, block));
      setCurrentPlayer(nextPlayer);
      setWhiteTime(wTime);
      setBlackTime(bTime);
      
      const win = checkWin(applyBlockToGrid(grid, block), block.player);
      if (win) {
  ...
  const getGhost = useCallback(() => {
    ...
    const isValid = validateMove(grid, validX, targetY, orientation, currentPlayer);
  ...
  }, [ghost, grid, blocks, currentPlayer, winner, gameMode, myPlayer, whiteTime, blackTime]);
    ...
    return (
      <div className="w-full h-screen bg-gray-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-gray-800 p-8 rounded-2xl shadow-2xl border border-gray-700">
           <h1 className="text-4xl font-bold text-white text-center mb-2 tracking-wider">CRACK COCAINE 3D</h1>
           <p className="text-gray-400 text-center mb-8">Structural Strategy Game</p>
           
           <div className="space-y-4">
              <button 
                onClick={() => { setGameMode('local'); setIsInLobby(false); }}
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-gray-500 text-sm">Online Multiplayer</span>
                <div className="flex-grow border-t border-gray-700"></div>
              </div>
              ...
              {connectionStatus === 'idle' || connectionStatus === 'error' ? (
                <>
                  <button 
                    onClick={() => { setGameMode('online'); startHost(); }}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                       onClick={() => {
                          const val = (document.getElementById('roomInput') as HTMLInputElement).value.toUpperCase();
                          if (val.length >= 4) {
                             setGameMode('online');
                             joinGame(val);
                          }
                  ...
                  <p className="text-[10px] text-gray-600 text-center mt-2">Requires local server (node server.js)</p>
                </>
              ) : (
                 <div className="text-center py-6 bg-gray-900 rounded-xl border border-gray-700 animate-in fade-in zoom-in duration-300">
                    {networkRole === 'host' ? (
                       <div className="space-y-4">
                          ...
                          <div className="text-4xl text-white font-mono font-bold tracking-[0.3em] bg-gray-950/50 py-2 rounded-lg">{roomId}</div>
                          <p className="text-xs text-gray-500">Share this code</p>
                          <button onClick={cancelHosting} className="text-red-400 text-xs hover:text-red-300 underline">Cancel</button>
                       </div>
                    ) : (
    ...
    <div className="relative w-full h-screen bg-gray-900 font-sans select-none overflow-hidden touch-none">
      ...
      <div className="absolute top-0 left-0 w-full p-4 pointer-events-none z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="bg-black/80 backdrop-blur-md px-6 py-3 rounded-xl text-white shadow-xl border border-white/10 flex items-center gap-6 md:gap-8 min-w-[300px] justify-between">
           ...
           {/* White Timer */}
           <div className={`flex items-center gap-3 transition-opacity duration-300 ${currentPlayer === 'white' && !winner ? 'opacity-100 scale-105' : 'opacity-40'}`}>
              <div className="relative">
                <div className={`w-4 h-4 rounded-full bg-amber-600 shadow-sm ${currentPlayer === 'white' && !winner ? 'ring-2 ring-white/50' : ''}`} />
                {currentPlayer === 'white' && !winner && (
                  <div className="absolute inset-0 bg-amber-400 rounded-full animate-ping opacity-75"></div>
                )}
              </div>
              <div className="flex flex-col">
                <span className={`font-mono text-xl font-bold ${whiteTime < 30 ? 'text-red-400' : 'text-white'}`}>
                  {formatTime(whiteTime)}
                </span>
                {gameMode === 'online' && (
                  <span className="text-[10px] text-gray-500 uppercase">{myPlayer === 'white' ? 'YOU' : 'OPP'}</span>
                )}
              </div>
           </div>
           ...
           <div className="text-white/20 font-bold text-sm flex flex-col items-center">
             <span>VS</span>
             {gameMode === 'online' && <span className="text-[10px] text-green-500 tracking-wider">LIVE</span>}
           </div>

           {/* Black Timer */}
           <div className={`flex items-center gap-3 transition-opacity duration-300 ${currentPlayer === 'black' && !winner ? 'opacity-100 scale-105' : 'opacity-40'}`}>
              <div className="flex flex-col items-end">
                <span className={`font-mono text-xl font-bold ${blackTime < 30 ? 'text-red-400' : 'text-white'}`}>
                  {formatTime(blackTime)}
                </span>
                {gameMode === 'online' && (
                  <span className="text-[10px] text-gray-500 uppercase">{myPlayer === 'black' ? 'YOU' : 'OPP'}</span>
                )}
              </div>
              <div className="relative">
                <div className={`w-4 h-4 rounded-full bg-neutral-800 border border-gray-500 shadow-sm ${currentPlayer === 'black' && !winner ? 'ring-2 ring-white/50' : ''}`} />
        ...
        <div className="absolute top-28 left-1/2 transform -translate-x-1/2 pointer-events-none z-20 animate-in fade-in zoom-in duration-500">
           <div className="bg-gradient-to-br from-green-500/90 to-emerald-600/90 backdrop-blur-md px-10 py-6 rounded-2xl shadow-2xl border-2 border-green-300/50 flex flex-col items-center">
              ...
              {gameMode === 'online' && winner === myPlayer && (
                 <div className="mt-2 text-xs bg-white/20 px-2 py-1 rounded text-white font-bold">VICTORY</div>
              )}
           </div>
        </div>
      )}
        ...
        {!winner && (
          <div className="hidden md:block text-white/50 text-xs bg-black/40 px-3 py-1 rounded backdrop-blur-sm">
             {gameMode === 'online' && currentPlayer !== myPlayer ? (
                <span className="text-yellow-400 animate-pulse">Opponent is thinking...</span>
             ) : (
        ...
        <div className="pointer-events-auto w-full max-w-lg flex items-center justify-between gap-4">
          ...
          {!winner ? (
            <>
              ...
              <button
                onClick={handlePlace}
                disabled={!ghost?.isValid || (gameMode === 'online' && currentPlayer !== myPlayer)}
                className={`flex-[2] h-14 font-bold rounded-2xl shadow-lg backdrop-blur-sm border border-white/10 transition-all active:scale-95 flex items-center justify-center gap-2
                  ${(ghost?.isValid && (gameMode !== 'online' || currentPlayer === myPlayer))
