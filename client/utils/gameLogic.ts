...
export const validateMove = (
  ...
  if (orientation === 'vertical') {
    ...
    if (y > 0) {
      ...
      if (cellBelow && cellBelow.player === player && cellBelow.orientation === 'vertical') {
    ...
    if (y > 0) {
      ...
      if (below && below.player === player && below.orientation === 'vertical') return false;
    }
  } else {
    ...
    if (x > 0) {
      ...
      if (left && left.player === player && left.orientation === 'horizontal') return false;
    }
    ...
    if (x + 2 < GRID_SIZE) {
      ...
      if (right && right.player === player && right.orientation === 'horizontal') return false;
    }
  }
 ...
 */
export const checkWin = (grid: GridState, player: Player): {x: number, y: number}[] | null => {
  // Directions: [dx, dy]
  const directions = [
    [1, 0],  // Horizontal
    [0, 1],  // Vertical
...
export const applyBlockToGrid = (grid: GridState, block: BlockData): GridState => {
  const newGrid = grid.map(col => [...col]); // Deep copy columns
  const { x, y, orientation, player, id } = block;

  const cellDataOrigin: CellData = { player, orientation, blockId: id, part: 'origin' };
  const cellDataExt: CellData = { player, orientation, blockId: id, part: 'extension' };
